...
import com.tech.chatappcompose.data.CHATS
...
class LCViewModel @Inject constructor(
        ...
        if (email.isEmpty() or password.isEmpty()) {
            ...
            auth.signInWithEmailAndPassword(email, password).addOnCompleteListener {
                if (it.isSuccessful) {
                    ...
                    handleException(customMessage = "Login Failed", exception = it.exception, context = context)
                }
            }
        }
    }

    private fun handleException(exception: Exception? = null, customMessage: String = "",context: Context ?= null) {
    ...
    fun onAddChat(number: String, context: Context) {
        if (number.isEmpty() or !number.isDigitsOnly()) {
            handleException(customMessage = "Number must be contain digits only.", context = context)
        } else {
                ...
                if (it.isEmpty) {
                    db.collection(USER_NODE).whereEqualTo("number", number).get()
                        .addOnSuccessListener {
                            if (it.isEmpty) {
                                handleException(customMessage = "Number not found", context = context)
                            } else {
                                ...
                                db.collection(CHATS).document(id).set(chat).addOnSuccessListener {
                                    Toast.makeText(context, "Number Added.", Toast.LENGTH_SHORT).show()
                                }.addOnFailureListener{
                                    handleException(customMessage = "Something went wrong", context = context)
                                }
                            }
                        }.addOnFailureListener {
                            handleException(it, context = context)
